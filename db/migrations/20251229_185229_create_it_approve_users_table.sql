...
CREATE TABLE IF NOT EXISTS sd_core.it_approve_users (
    ...
    task_text                TEXT,
    flag_ignored             BOOLEAN NOT NULL DEFAULT FALSE
);
...
COMMENT ON COLUMN sd_core.it_approve_users.id_order_state IS 'Состояние согласованта';
...
COMMENT ON COLUMN sd_core.it_approve_users.date_fact IS 'Фактическая дата согласования(Выполнения задачи)';
...
CREATE INDEX idx_approve_users_id_order_state ON sd_core.it_approve_users (id_order_state);
...
CREATE INDEX idx_approve_users_user_role ON sd_core.it_approve_users (id_user_role) WHERE id_user_role IS NOT NULL;
