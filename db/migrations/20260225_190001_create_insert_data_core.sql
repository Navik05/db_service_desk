-- Migration: create_insert_data_core
-- Created: Fri Feb  6 13:18:47     2026
BEGIN;

-- С внешними ключами
INSERT INTO
    sd_core.it_service (
        fullname,
        sname,
        description,
        developer,
        date_s,
        date_f,
        priznak_is,
        id_service_type,
        id_service_state,
        id_exp_type,
        id_service_parent,
        is_need_approval,
        is_service,
        business_critical,
        basis_s
    )
VALUES
    (
        'Сервис технической поддержки пользователей',
        'стпп',
        'Описание сттп',
        'Разработчик сттп сервиса',
        '2026-01-29 20:00:00-07',
        null,
        false,
        2,
        1,
        2,
        null,
        false,
        false,
        1,
        'Какой-то базис'
    ),
    (
        '1С: Предприятие',
        '1С: Предприятие',
        'Описание 1cпред',
        'Разработчик 1cпред сервиса',
        '2026-01-29 20:00:00-07',
        null,
        false,
        1,
        1,
        2,
        null,
        false,
        false,
        3,
        'Какой-то базис'
    ),
    (
        '1С: Выписки 7.7',
        '1С: Выписки 7.7',
        'Автоматизированная система  для автоматизации процесса разноски банковских выписок из Банк-клиентов в ПК Казна, 1С:Бухгалтерия и ПК Реализация.',
        'ООО "1С", АО "Алтайэнергосбыт"',
        '2009-01-01 20:00:00-07',
        '2013-06-01 20:00:00-07',
        true,
        2,
        3,
        2,
        2,
        true,
        true,
        2,
        'Какой-то базис'
    ),
    (
        'Агентские договора',
        'Агентские договора',
        'Описание Агентские договора',
        'Разработчик Агентские договора сервиса',
        '2026-01-29 20:00:00-07',
        null,
        false,
        1,
        null,
        2,
        null,
        false,
        false,
        3,
        'Какой-то базис'
    ),
    (
        '1С: ЖКХ (МУП "Теплосбыт" - ФЛ)',
        '1С: ЖКХ (МУП "Теплосбыт" - ФЛ)',
        'Описание 1С: ЖКХ (МУП "Теплосбыт" - ФЛ)',
        'Разработчик 1С: ЖКХ (МУП "Теплосбыт" - ФЛ) сервиса',
        '2020-07-01 20:00:00-07',
        null,
        true,
        1,
        1,
        2,
        null,
        true,
        true,
        1,
        'Какой-то базис'
    );

INSERT INTO
    sd_core.it_catalogitem (
        id_service,
        id_catitem_parent,
        id_exp_type,
        exp_basis,
        exp_date,
        nomer,
        name,
        description,
        info,
        id_effect,
        id_scale
    )
VALUES
    (
        1,
        null,
        1,
        null,
        null,
        '0000001',
        'Первая услуга',
        'Описание первой услуги',
        'Инфо первой услуги',
        1,
        1
    ),
    (
        2,
        null,
        1,
        null,
        null,
        '0000001',
        'Вторая услуга',
        'Описание второй услуги',
        'Инфо второй услуги',
        1,
        1
    );


INSERT INTO
    sd_core.it_user (
        login_ad,
        email_ad,
        tel_ad,
        fio_1c,
        id_podr,
        dolzh_1c,
        tab_num_1c,
        d_c,
        is_user,
        d_prin,
        d_uvol,
        agreement_type,
        fiz_lico,
        state_1c,
        cti_user,
        cti_password,
        crm_user,
        crm_password,
        inter_ad,
        grade
    )
VALUES
    (
        'user1',
        'user1@mail.ru',
        '+1(111)111-1111',
        'Арбузов Александр Александрович',
        1,
        'Первая должность 1с',
        'Первый табномер 1с',
        '2026-01-29',
        true,
        '2026-01-29',
        null,
        'Первый тип соглашения',
        'Первый код физлица 1с',
        'Первый статус сотрудника 1с',
        'arbuzovcti',
        'arbuzovpasscti',
        'arbuzovcrm',
        'arbuzovpasscrm',
        true,
        1
    ),
    (
        'user2',
        'user2@mail.ru',
        '+2(222)222-2222',
        'Борисов Борис Борисович',
        1,
        'Вторая должность 1с',
        'Второй табномер 1с',
        '2026-01-29',
        true,
        '2026-01-29',
        null,
        'Второй тип соглашения',
        'Второй код физлица 1с',
        'Второй статус сотрудника 1с',
        'borisovcti',
        'borisovpasscti',
        'borisovcrm',
        'borisovpasscrm',
        true,
        2
    ),
    (
        'user3',
        'user3@mail.ru',
        '+3(333)333-3333',
        'Володин Владимир Владимирович',
        1,
        'Третья должность 1с',
        'Третий табномер 1с',
        '2026-01-29',
        true,
        '2026-01-29',
        null,
        'Третий тип соглашения',
        'Третий код физлица 1с',
        'Третий статус сотрудника 1с',
        'volodincti',
        'volodinpasscti',
        'volodincrm',
        'volodinpasscrm',
        true,
        3
    );

INSERT INTO
    sd_core.it_order (
        name,
        description,
        date_c,
        date_f_plan,
        date_f_fact,
        id_order_parent,
        id_order_type,
        id_catitem,
        id_service,
        id_order_state,
        id_order_priority,
        id_user_creator,
        id_user_initiator,
        id_user_dispatcher,
        id_order_source,
        result_text,
        id_user_executor,
        date_postpone,
        comment
    )
VALUES
    (
        'Очень большой такой заголовок заявки',
        'Длинное описание Длинное описание Длинное описание Длинное описание',
        '2026-01-29 20:00:00-07',
        '2026-02-01 20:00:00-00',
        null,
        null,
        3,
        1,
        3,
        10,
        1,
        1,
        2,
        null,
        1,
        null,
        null,
        '2026-02-014 10:00:00-00',
        'нужно починить принтер'
    );

INSERT INTO sd_core.it_group (id_user, name, name_1c_doc, description)
    VALUES (1, 'Группа поддержки', 'ГрПоддержки', 'Группа для решения оперативных задач'),
            (2, 'Группа разработки', 'ГрРазработки', 'Команда разработки новых функций');

INSERT INTO sd_core.it_user_group (id_group, id_user) 
    VALUES (1, 1), (1, 2);

INSERT INTO sd_core.it_user_authority (id_user, id_authority) 
    VALUES (1, 1), (1, 2);

INSERT INTO sd_core.it_podr_fact_location (id_podr, id_fact_location) 
    VALUES(1,3), (1,2), (1,1), (2,2);

INSERT INTO sd_core.it_user_role(name)
    VALUES('Куратор от ИТ'), ('Сервис-менеджер'), ('Функциональный
заказчик'), ('Держатель сервиса'), ('Согласующий');

INSERT INTO sd_core.it_catitem_user_role (id_catitem, id_user, id_podr, id_user_role, id_service)
    VALUES (1, 1, 1, 1, 1), (1, 2, 2, 2, 2), (1, 3, 2, 3, 3), (2, 1, 2, 1, 4), (2, 2, 1, 4, 5),
            (1, 1, 2, 1, 3), (2, 2, 2, 2, 3), (1, 1, 1, 1, 5);

COMMIT;